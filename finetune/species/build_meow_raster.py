...
def _write_hierarchy(output_dir: Path) -> Path:
...
def build_from_shapefile(shp_path: Path, output_dir: Path) -> tuple[Path, Path]:
    """
    ...
    try:
        ...
        raise ImportError(
            "Instala: pip install geopandas rasterio shapely"
        )
    ...
    shapes = [
        (mapping(row.geometry), int(row[eco_col]))
        for _, row in meow.iterrows()
...
def main() -> None:
    ...
    if args.mock or args.shapefile is None:
        ...
        bin_p, hier_p = build_mock_raster(args.output_dir)
    else:
        print(f"Rasterizando shapefile: {args.shapefile}...")
        bin_p, hier_p = build_from_shapefile(args.shapefile, args.output_dir)
